...
const EPS = 1e-3; // treat anything <= 0.001 as zero
...
/** Remove scraps that have no remaining resources and clear selection if needed. */
function pruneEmptyScraps(){
  if (!Array.isArray(state.scraps) || !state.scraps.length) return;
  const before = state.scraps.length;
  state.scraps = state.scraps.filter((s:any)=> (Number(s.b||0) + Number(s.e||0)) > EPS);
  if (before !== state.scraps.length && state.selectedScrapId){
    if (!state.scraps.some((s:any)=> s.id === state.selectedScrapId)) state.selectedScrapId = null;
  }
}
...
export function accrue(updateHud:()=>void){
  ...
  const moonRate = state.moons.reduce((s:number,m:any)=>s+(m.lvl??1),0);
  const satRate  = state.sats .reduce((s:number,m:any)=>s+(m.lvl??1),0);
  ...
  for(const m of state.missions){
    if(m.phase==='out' && now >= m.arrive){
      ...
      if(sc){
        ...
        const avail = Math.max(0, (sc.b||0) + (sc.e||0));
        const take = Math.min(capacity, avail);
        const takeB = Math.min(sc.b||0, Math.round(take * ((sc.b||0)/(((sc.b||0)+(sc.e||0))||1))));
        const takeE = Math.min(sc.e||0, take - takeB);
        m.toCollect = { b: takeB, e: takeE };
        m.collected = m.collected || { b:0, e:0 };
        m._rateB = takeB / ECON.collectionSeconds;
        m._rateE = takeE / ECON.collectionSeconds;
      } else {
        m.collectEnd = now; m._rateB = m._rateE = 0;
      }
    }
    ...
    if(m.phase==='collect'){
      ...
      if(sc){
        sc.b = Math.max(0, (sc.b||0) - stepB);
        sc.e = Math.max(0, (sc.e||0) - stepE);
        // clamp tiny float dust to 0
        if (sc.b < EPS) sc.b = 0;
        if (sc.e < EPS) sc.e = 0;

        // immediate removal if depleted
        if((sc.b + sc.e) <= EPS){
          state.scraps = state.scraps.filter((s:any)=>s.id!==sc.id);
          if (state.selectedScrapId === sc.id) state.selectedScrapId = null;
        }
      }

      m.collected.b = (m.collected.b||0) + stepB;
      m.collected.e = (m.collected.e||0) + stepE;

      if(now>=m.collectEnd){
        m.phase='back';
        m.leave=m.collectEnd;
        m.return=m.leave + (m.arrive - m.depart);
      }
    }
  ...
  if(state.missions.some((m:any)=>m.phase==='done')){
    for(const _m of state.missions.filter((m:any)=>m.phase!=='done')){} // keep structure
    for(const m of state.missions.filter((m:any)=>m.phase==='done')){
      ...
      state.ships.drone  += m.drones;
      state.ships.battle += m.battles;
    }
    state.missions = state.missions.filter((m:any)=>m.phase!=='done');
    updateHud();
  }

  // safety net: in case anything left a near-zero remainder
  pruneEmptyScraps();
}
