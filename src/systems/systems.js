...
export function orbitSpeed(r){ return ORBIT.speedBase * (100 / Math.max(50, r)); }
export function upgradeCost(kind, lvl){
...
export function capacityOf(d,b){ return d*ECON.ships.drone.cap + b*ECON.ships.battle.cap; }

export function accrue(updateHud){
  const now = Date.now();
  ...
  for(const m of state.missions){
    ...
    if(m.phase==='collect'){
      ...
      if(sc){
        ...
        if(sc.b<=0 && sc.e<=0){ state.scraps = state.scraps.filter(s=>s.id!==sc.id); }
      }
  ...
  if(state.missions.some(m=>m.phase==='done')){
    ...
    state.missions = state.missions.filter(m=>m.phase!=='done'); updateHud();
  }
}

export function enqueueShip(type, toast, updateHud){
...
// placement helpers
export function isPlacementValid(r){
